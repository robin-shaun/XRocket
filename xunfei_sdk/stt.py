...
def speech_to_text(waveData):
    ...
    if plat[0] == '32bit':
        cur = cdll.LoadLibrary(BASEPATH + '/libs/x86/libmsc.so')
    else:
        cur = cdll.LoadLibrary(BASEPATH + '/libs/x64/libmsc.so')
    ...
    ret = MSPLogin(None,None,'appid = 5e2faa83, work_dir = .') 
    if ret != 0:
...
if __name__ == '__main__':
    speech_to_text(getWaveData('3.wav'))
